...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($result) {
        ...
        if(mysqli_num_rows($result) > 0){
            ...
            $hashedPassword = $row["company_password"];
            ...
            if(password_verify($password, $hashedPassword)){
                ...
                $_SESSION["AuthenticatedUser"] = [
                    ...
                    'EmployerEmail' => $row["company_email"]
                ];

                $Name = $_SESSION["AuthenticatedUser"]["EmployerName"];
                $Employer_ID = $_SESSION["AuthenticatedUser"]["EmployerID"];
               
                // Check if profile completion record exists
                $checkQuery = "SELECT COUNT(*) AS count FROM company_profile_completion WHERE company_id = '$Employer_ID'";
                $checkResult = mysqli_query($conn, $checkQuery);
                $count = mysqli_fetch_assoc($checkResult)['count'];

                if ($count == 0) {
                    // No record exists, insert a new record indicating incomplete profile
                    $insertQuery = "INSERT INTO company_profile_completion (company_id, profile_completed) VALUES ('$Employer_ID', 0)";
                ...
                if($profileCompleted){
                    echo json_encode(array("member" => true, "message" => "Welcome Back" . " " . $Name));
                }else{
