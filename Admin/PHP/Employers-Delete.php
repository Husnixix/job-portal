...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($data !== null) {
        ...
        $tables = array("company_about_me", "company_job_posts", "company_profile_completion", "event_calender", "job_applications");
        ...
        foreach ($tables as $table) {
            ...
            if ($count > 0) {
                ...
                $delete_query = "DELETE FROM $table WHERE company_id = '$EmployerID'";
                mysqli_query($conn, $delete_query);
            }
        }

        // Now delete user from companies table
        $delete_employer_query = "DELETE FROM companies WHERE id = '$EmployerID'";
        if (mysqli_query($conn, $delete_employer_query)) {
            echo json_encode(array("success" => true, "message" => "Employer Deleted"));
        } else {
