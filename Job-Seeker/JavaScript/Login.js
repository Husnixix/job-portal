...
const candidateLoginForm = document.querySelector(".form");
...
candidateLoginForm.addEventListener('submit', function(event){
    if (!validateForm()){
        ...
        request.open("POST", "http://localhost/STAPH/Job-Seeker/PHP/Login-Process.php", true);
        request.onload = function(){
            if (request.status >= 200 && request.status < 400) {
                ...
                if (response.member) { // if Complete profile true
                    ...
                    successMessage(response.message);
                    window.location.href = "http://localhost/STAPH/Job-Seeker/Profile/About.html";
                    clearFormFields();
        ...
        var formData = new FormData(candidateLoginForm);
